...
namespace BravaSystem.Communication
{
	...
	public class BravaCodes
	{
        ...
        // The different Datasets possible in the Electricity metering function.
	...
	public class RequestType : BravaTransaction
	{
		...
		public void ParseResultStream()
		{
            ...
            if ( (m_resStrm.Length == (BravaConstants.INFOMESSAGERECORDLENGTH + 5)) &&
                 (bufferBytes[0] == 0x01) ) // First byte must always be '0x01': Number Of Records
			{
				if ((bufferBytes[1] == (byte)BravaCodes.Tasks.Response) &&
					(bufferBytes[2] == (byte)BravaCodes.Functions.Gateway) &&
					(bufferBytes[3] == (byte)BravaCodes.GatewayRecords.TypeString ) &&
					(bufferBytes[4] == BravaCodes.AllRecords))
				{
					...
					writer.WriteAttributeString("Task", XmlConvert.ToString(bufferBytes[1]));
					writer.WriteAttributeString("Function", XmlConvert.ToString(bufferBytes[2]));
					writer.WriteAttributeString("RecordType", XmlConvert.ToString(bufferBytes[3]));
					writer.WriteAttributeString("RecordIndex", XmlConvert.ToString(bufferBytes[4]));
					writer.WriteEndElement();  // End "Header"

					writer.WriteStartElement("Data");
					writer.WriteAttributeString("RecordCount", XmlConvert.ToString(bufferBytes[0]));
	...
	public class RequestReadings : BravaTransaction
	{
		...
		public void ParseResultStream()  
		{
            ...
            byte b_numRecords = bufferBytes[0];
			...
			{ 
                ...
                if ((bufferBytes[1] == (byte)BravaCodes.Tasks.Response) &&
                    (bufferBytes[2] == (byte)BravaCodes.Functions.Gateway) &&
                    (bufferBytes[3] == (byte)BravaCodes.GatewayRecords.MeterReadings) &&
                    (bufferBytes[4] == BravaCodes.AllRecords))
				{ 
    ...
    public class RequestStateBlock : BravaTransaction
    {
        ...
        public void ParseResultStream()
        {
            ...
            if (m_resStrm.Length == ((b_numRecords * BravaConstants.STATEBLOCKRECORDLENGTH) + 5))
            {
                ...
                if ((bufferBytes[1] == (byte)BravaCodes.Tasks.Response) &&
                    (bufferBytes[2] == (byte)BravaCodes.Functions.Gateway) &&
                    (bufferBytes[3] == (byte)BravaCodes.GatewayRecords.StateBlock) &&
                    (bufferBytes[4] == BravaCodes.AllRecords))
                {
                    // Response Header Correlates with Request header.
                    XmlTextWriter writer = new XmlTextWriter(resMemStr, Encoding.UTF8);
                    ...
                    for (int recnum = 0; recnum < b_numRecords; recnum++)
                    {
                        ...
                        switch (cRecordIndex)
                        {
                            ...
                            case (byte)BravaCodes.StateBlockRecords.SwitchBankLS8:
                                ...
                                writer.WriteAttributeString("Circuit2State", (bufferBytes[5 + (recnum * BravaConstants.STATEBLOCKRECORDLENGTH) + 1 + 7]) == 0x12 ? "Connected":"Disconnected");
                                writer.WriteAttributeString("Circuit3State", (bufferBytes[5 + (recnum * BravaConstants.STATEBLOCKRECORDLENGTH) + 1 + 8]) == 0x12 ? "Connected":"Disconnected");
                                writer.WriteAttributeString("Circuit4State", (bufferBytes[5 + (recnum * BravaConstants.STATEBLOCKRECORDLENGTH) + 1 + 9]) == 0x12 ? "Connected":"Disconnected");
                                writer.WriteAttributeString("Circuit5State", (bufferBytes[5 + (recnum * BravaConstants.STATEBLOCKRECORDLENGTH) + 1 + 10]) == 0x12 ? "Connected":"Disconnected");
                                writer.WriteAttributeString("Circuit6State", (bufferBytes[5 + (recnum * BravaConstants.STATEBLOCKRECORDLENGTH) + 1 + 11]) == 0x12 ? "Connected":"Disconnected");
                                writer.WriteAttributeString("Circuit7State", (bufferBytes[5 + (recnum * BravaConstants.STATEBLOCKRECORDLENGTH) + 1 + 12]) == 0x12 ? "Connected":"Disconnected");
                                writer.WriteAttributeString("Circuit8State", (bufferBytes[5 + (recnum * BravaConstants.STATEBLOCKRECORDLENGTH) + 1 + 13]) == 0x12 ? "Connected":"Disconnected");
                                break;
                        }
